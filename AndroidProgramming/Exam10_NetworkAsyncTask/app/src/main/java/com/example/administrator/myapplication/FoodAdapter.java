...
public class FoodAdapter extends BaseAdapter { //BaseAdapter는 추상메소다가 있는 추상 클래스
    ...
    // ■ food_item.xml에 있는 걸 객체화 시키기(인플레이션) 위해서 필요한 요소들
    ...
    /*
    public FoodAdapter(MainActivity mainActivity){
    ...
    */
    ...
    {
        //food_item.xml에 있는 것을 인플레이션해서 Food 데이터를 셋팅하고 리턴해주는 메소드

        //이 메소드는 리스트안에 들어가는 객체의 수만큼 호출됨, 즉,
        //int position : 1->2->3 식으로 들어옴
        //View convertView : null이 들어오고 안들어오고(재사용이냐 아니냐에 따라 )
        //                   food_item.xml를 가지로 인플레이션 한거, 최초 처음 인플레이션을 만드는 동안에는 null이 들어오고, 추후 재사용할게 발생하면 그 인플레이션한게 들어옴
        ...
        if(convertView == null){
            //inplation 인플레이션
            convertView = layoutInflater.inflate(R.layout.food_item, null); //인플레이션
        }

        //데이터 셋팅 data setting
        Food food = list.get(position); //데이터
        ImageView foodImage = (ImageView) convertView.findViewById(R.id.foodImage);
        foodImage.setImageBitmap(food.getImage()); //food.getImage() : food객체(데이터) 안에있는 이미지를 반환

        TextView foodTitle = (TextView) convertView.findViewById(R.id.foodTitle);
        foodTitle.setText(food.getTitle());  //food.getTitle() : food객체(데이터) 안에있는 텍스트를 반환

        TextView foodPrice = (TextView) convertView.findViewById(R.id.foodPrice);
        foodPrice.setText(food.getPrice());  //food.getTitle() : food객체(데이터) 안에있는 텍스트를 반환

        TextView foodContent = (TextView) convertView.findViewById(R.id.foodContent);
        foodContent.setText(food.getContent());  //food.getContent() : food객체(데이터) 안에있는 텍스트를 반환
