	...
	public static void main(String[] args)  throws Exception{
		...
		while(true){
			...
			if ( strMenuChoice.equals("1") ){
				...
				for(int i=0; i<BoardDB.length; i++){
					if(BoardDB[i] != null){
						if(Integer.parseInt(strIndex) == BoardDB[i].getNo() ){
							//해당 게시글이 존재할경우 출력
							//@조회했으니 카운터 +1
							BoardDB[i].plusCnt(); 
							//@상세보기 출력
							System.out.println("\t----------------------------------------------------------------------------");
							System.out.println( "\t■제목 : " + BoardDB[i].getSubject()  );
							System.out.print( "\t■번호 : " + BoardDB[i].getNo() );
							System.out.print( "\t ■글쓴이 : " + BoardDB[i].getWriter()  );
							System.out.print( "\t ■조회수 : " + BoardDB[i].getCnt()  );
							System.out.println();
